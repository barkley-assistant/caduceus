...
### Task 8.1: Reference worker: OpenCode + Gentle-AI

**Objective:** Document and provide a tested reference worker that wraps `opencode run`.

**Files:**
- Create: `examples/worker-opencode-sdd.sh`

**Step 1:** Write the wrapper script that reads `CADUCEUS_*` env vars, constructs the `opencode run` invocation, and exits with the worker's code.

**Step 2:** Document in the README's "Reference Worker" section (already in v0.1 README).

**Step 3:** Commit: `docs(examples): opencode + gentle-ai reference worker`

### Task 8.2: Investigation worker

**Objective:** Document a thinner worker for `🤖 auto-fix-investigate` tickets.

**Files:**
- Create: `examples/worker-investigate.py`

**Step 1:** Write a Python worker that calls GitHub API for issue context (using a separate bot token), writes `findings.md` and a minimal `sdd-result.json`, and exits 0.

**Step 2:** Document in README.

**Step 3:** Commit: `docs(examples): investigation worker`
