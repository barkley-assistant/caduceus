...
**Goal:** Build a self-hosted Rust daemon that polls GitHub for labeled issues, manages an atomic claim queue, provisions isolated git worktrees, spawns a pluggable worker (OpenCode + Gentle-AI for v0.1), enforces a hard worker timeout, and finalizes the result as a branch + push + PR + issue close.

**Architecture:** Strict controller-worker separation. The Rust daemon owns process lifecycle, IO, atomicity, and observability. The worker is a single command that reads `CADUCEUS_*` env vars, edits files in the worktree, writes `sdd-result.json`, and exits with a code. The daemon never sees an LLM API key or token.
...
- `ulid` (run IDs)
- Reference worker: OpenCode + Gentle-AI (`gentle-orchestrator` agent)
...
The default `comment_forbidden_strings` list is exactly: `["caduceus", "opencode", "gentle-ai", "engram", "sdd"]`. **Explicit user values REPLACE the defaults** — they do not merge. This is to keep the rule strict: a user who lists one forbidden string is signaling they have thought about the rule and want that exact list.
...
**Objective:** Implement the config lookup order: `$CADUCEUS_CONFIG` env var → `~/.config/caduceus/config.yaml` → `~/.hermes/config.yaml` (under `caduceus:` section).
...
fn caduceus_config_env_var_wins() {
    ...
    let standalone_path = standalone.path().join("config.yaml");
    std::fs::write(&standalone_path, "caduceus:\n  poll_user: \"from-standalone\"\n  worker_command: [\"x\"]\n").unwrap();

    let cfg = caduceus::config::Config::load_with_overrides(Some(&standalone_path), None).unwrap();
...
#[test]
fn standalone_config_used_when_no_env_var() {
    std::env::remove_var("CADUCEUS_CONFIG");
    let standalone = tempdir().unwrap();
...
fn hermes_fallback_uses_caduceus_subsection() {
    ...
    let cfg = caduceus::config::Config::load_with_overrides(None, Some(&path)).unwrap();
    assert_eq!(cfg.poll_user, "from-hermes");
}

#[test]
fn explicit_standalone_overrides_hermes_fallback() {
    std::env::remove_var("CADUCEUS_CONFIG");
    let hermes = tempdir().unwrap();
    ...
    let cfg = caduceus::config::Config::load_with_overrides(Some(&standalone_path), Some(&hermes_path)).unwrap();
    assert_eq!(cfg.poll_user, "from-standalone");
}
```

**Step 2:** Run tests, verify FAIL.

**Step 3:** Implement `Config::load_with_overrides(standalone_path: Option<&Path>, hermes_path: Option<&Path>) -> Result<Self>` and a higher-level `Config::load() -> Result<Self>` that does the full resolution chain from the decision (env var → standalone → hermes fallback).

**Step 4:** Run tests, verify PASS.

**Step 5:** Commit: `feat(config): standalone-first with Hermes fallback resolution chain`
...
**Objective:** Given an `owner/repo` slug, locate the on-disk clone at `<sdd_workdir_base>/<owner>/<repo>`.
...
**Objective:** Create a fresh worktree at `<sdd_workdir_base>/<owner>/<repo>/.worktrees/<branch>/` from the default branch.
...
### Task 5.3: sdd-result.json parsing

**Objective:** After a successful exit-0 worker run, parse `<worktree>/sdd-result.json` and validate the schema.

**Files:**
- Modify: `src/worker.rs`
- Modify: `src/finalize.rs`

**Step 1:** Test with a valid JSON file and verify parsed fields. Test with missing field, invalid JSON, missing file — all should error gracefully.

**Step 2:** Implement `SddResult::from_path(path: &Path) -> Result<Self>` with serde-derived struct.

**Step 3:** Commit: `feat(worker): sdd-result.json parsing`
...
**Objective:** Use the GitHub REST API to open a PR with title and body from `sdd-result.json`.
...
**Step 2:** Test the loop end-to-end against a mocked GitHub API and a trivial worker that writes `sdd-result.json`.
...
### Task 8.1: Reference wrapper script (OpenCode CLI bridge)

**Objective:** Provide a tested, documented wrapper script that translates Caduceus's `CADUCEUS_*` env vars into a clean OpenCode CLI invocation. The wrapper is **purely a translation layer** — no domain logic. Users who want to call `opencode` directly via `worker_command` can skip it.

**Files:**
- Create: `examples/worker-opencode-sdd.sh`

**Step 1:** Write a wrapper script that:

- Reads `$CADUCEUS_WORKTREE_PATH` and `cd`s into it
- Reads `$CADUCEUS_RUN_ID` and writes it to `<state_dir>/runs/<run_id>.heartbeat` every 30 seconds while running
- Reads `$CADUCEUS_ISSUE_LABELS` and uses it to set the appropriate behavior flag for the orchestrator
- Constructs the `opencode run --agent gentle-orchestrator -f .hermes-sdd-prompt.txt -- "..."` invocation
- Exits with OpenCode's exit code (so Caduceus's `worker_timeout_seconds` and transcript capture work correctly)

**Step 2:** Document the wrapper's behavior in README — note that it's optional, and that direct `worker_command` invocation is supported without it.

**Step 3:** Add a test that runs the wrapper against a mock OpenCode (a script that sleeps 2s and exits 0) and verifies:
- Exit code propagates correctly
- The heartbeat file is written
- The cwd is set correctly

**Step 4:** Commit: `docs(examples): opencode CLI bridge wrapper`
...
5. **Bot account comment voice: hard rule.** The daemon enforces the public-voice rule by scanning every outbound bot comment against a forbidden-string list before posting. If a match is found, the post is rejected and an error is logged. Default forbidden strings: `caduceus`, `opencode`, `gentle-ai`, `engram`, `sdd`. The list is **not** hardcoded — users can extend or override it via `comment_forbidden_strings` in config (see Task 6.5).

6. **Config file location: standalone-first with Hermes fallback.** v0.1 resolves config in this order:
   1. `$CADUCEUS_CONFIG` environment variable (path to a YAML file)
   2. `~/.config/caduceus/config.yaml` (XDG-style, the recommended location for standalone users)
   3. `~/.hermes/config.yaml` under the `caduceus:` section (Hermes integration; the daemon reads only the `caduceus:` subsection)

   This lets standalone users run Caduceus without Hermes installed, while Hermes users can co-locate the config alongside their other agent settings. A small Hermes plugin (see `plugin/README.md`) provides auto-discovery and `caduceus status` integration, but is fully optional.
