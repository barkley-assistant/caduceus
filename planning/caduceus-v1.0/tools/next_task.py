#!/usr/bin/env python3
"""Return the single next eligible task or phase gate for the v1.0 plan."""
...
def select_next(manifest: dict, progress: dict) -> dict:
    if manifest["catalog_status"] == "draft":
        return {
            ...
            "reason": "v1.0 task catalog is draft",
            "next_action": (
                "write and review the complete task catalog, add acceptance IDs, "
                "then set catalog_status to active"
            ),
        }
