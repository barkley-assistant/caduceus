...
//!
//! Verifies that the dispatch in `src/daemon/tick/per_claim.rs` calls
...
fn tick_calls_executor_run_not_process_supervise() {
    ...
    let tick_path = format!("{project_root}/src/daemon/tick/per_claim.rs");
    ...
    assert!(
        ...
        "src/daemon/tick/per_claim.rs must call services.executor.run; \
    ...
    assert!(
        ...
        "src/daemon/tick/per_claim.rs must NOT call services.process.supervise; \
